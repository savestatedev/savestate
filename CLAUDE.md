...
- `savestate stats [--json] [--adapter] [--exclude] [--since] [--until]` — engagement loop: shows total snapshots, time
  covered, cadence, adapter mix, top tags. `--adapter` limits stats to one known adapter. `--exclude` skips one or more known adapters. `--since` / `--until` limit stats to an ISO 8601 date range.
