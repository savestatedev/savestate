...
describe('savestate import checksum wrong-password output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-checksum-wrong-pass-${Date.now()}`);
  ...
  async function writeContainer(fileName: string, sha256?: unknown): Promise<{ filePath: string; checksum?: string }> {
    ...
    const manifest: Record<string, unknown> = {
      formatVersion: 1,
      created: '2026-08-25T01:00:00.000Z',
      agentId: 'fixture-agent',
      encryption: {
  ...
  it('prints the payload checksum when the passphrase is wrong', async () => {
    const { filePath, checksum } = await writeContainer('wrong-pass-checksum.savestate');
  ...
  it('omits the checksum line when the passphrase is wrong and sha256 is missing', async () => {
    const { filePath } = await writeContainer('wrong-pass-missing-checksum.savestate', null);
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
