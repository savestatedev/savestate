...
describe('savestate verify empty keyfile', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = await mkdtemp(join(tmpdir(), 'savestate-verify-empty-keyfile-'));
  });
  ...
  it('rejects an empty keyfile before decrypting', async () => {
    const filePath = await writeContainer('empty-keyfile.savestate', {
      passphrase: 'synthetic-verify-pass',
    });

    const result = await verifyContainer(filePath, { keyfile: '' });
  ...
  it('rejects a whitespace-only keyfile before decrypting', async () => {
    const filePath = await writeContainer('whitespace-keyfile.savestate', {
      passphrase: 'synthetic-verify-pass',
    });
  ...
  it('still verifies a non-empty keyfile path', async () => {
