...
describe('savestate import excluded corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-excluded-corrupted-${Date.now()}`);
  ...
  it('prints excluded paths when the file is corrupted', async () => {
    const filePath = await writeContainer('corrupted-excluded.savestate', ['personality']);
  ...
  it('omits the excluded line when the file is corrupted and the archive has none', async () => {
    const filePath = await writeContainer('corrupted-missing-excluded.savestate');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
