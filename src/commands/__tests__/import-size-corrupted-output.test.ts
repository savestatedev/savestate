...
describe('savestate import size corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-size-corrupted-${Date.now()}`);
  ...
  it('prints the payload size when the file is corrupted', async () => {
    const { filePath, payloadBytes } = await writeContainer('corrupted-size.savestate');
  ...
  it('omits the size line when the file is corrupted and byteLength is missing', async () => {
    const { filePath } = await writeContainer('corrupted-missing-size.savestate', null);
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
