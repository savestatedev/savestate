...
describe('savestate doctor --tag', () => {
  ...
  it('keeps snapshots that include the tag', () => {
    expect(
      resolveDoctorSnapshots(
        [
          { id: 'work', timestamp: '2026-01-01T00:00:00Z', adapter: 'chatgpt', tags: ['work'] },
          { id: 'personal', timestamp: '2026-05-01T00:00:00Z', adapter: 'chatgpt', tags: ['personal'] },
        ],
        { tag: 'work' },
      ).map((entry) => entry.id),
    ).toEqual(['work']);
  ...
  it('ANDs --adapter with --tag', () => {
    expect(
      resolveDoctorSnapshots(
        [
          { id: 'work-gpt', timestamp: '2026-01-01T00:00:00Z', adapter: 'chatgpt', tags: ['work'] },
          { id: 'work-claude', timestamp: '2026-05-01T00:00:00Z', adapter: 'claude-code', tags: ['work'] },
        ],
