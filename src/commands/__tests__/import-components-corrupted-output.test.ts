...
describe('savestate import components corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-components-corrupted-${Date.now()}`);
  ...
  async function writeContainer(fileName: string, components?: unknown): Promise<string> {
    ...
    const plaintext = Buffer.from(JSON.stringify({
      ...
      version: 1,
      exportedAt: '2026-08-25T04:00:00.000Z',
    ...
    const manifest: Record<string, unknown> = {
      formatVersion: 1,
      created: '2026-08-25T04:00:00.000Z',
      agentId: 'fixture-agent',
      payloads: [
        {
          name: 'agent_state',
  ...
  it('prints included components when the file is corrupted', async () => {
    const filePath = await writeContainer('corrupted-components.savestate', ['memory', 'tools']);
  ...
  it('omits the components line when the file is corrupted and the archive has none', async () => {
    const filePath = await writeContainer('corrupted-missing-components.savestate');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
