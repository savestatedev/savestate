...
import { formatImportInput, importState } from '../container.js';
...
describe('savestate import input wrong-password output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-input-wrong-pass-${Date.now()}`);
  ...
  async function writeContainer(fileName: string): Promise<string> {
    ...
    const plaintext = Buffer.from(JSON.stringify({
      ...
      version: 1,
      exportedAt: '2026-08-25T06:00:00.000Z',
    ...
    const manifest: Record<string, unknown> = {
      formatVersion: 1,
      created: '2026-08-25T06:00:00.000Z',
      agentId: 'fixture-agent',
      payloads: [
        {
          name: 'agent_state',
  ...
  it('prints the input path when the passphrase is wrong', async () => {
    const filePath = await writeContainer('wrong-pass-input.savestate');
    ...
    expect(error.mock.calls.flat()).toContain(formatImportInput(filePath));
  ...
  it('omits an input line when the file is not a SaveState container', async () => {
    ...
    expect(error.mock.calls.flat().some((line) => typeof line === 'string' && line.includes('Input:'))).toBe(false);
    expect(log.mock.calls.flat().some((line) => typeof line === 'string' && line.includes('Input:'))).toBe(false);
    error.mockRestore();
    log.mockRestore();
    exit.mockRestore();
  });
