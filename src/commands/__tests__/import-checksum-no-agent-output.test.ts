...
describe('savestate import checksum no-agent output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-checksum-no-agent-${Date.now()}`);
  ...
  async function writeContainer(fileName: string, sha256?: unknown): Promise<{ filePath: string; checksum?: string }> {
    ...
    const plaintext = Buffer.from(JSON.stringify({
      ...
      version: 1,
      exportedAt: '2026-08-25T17:00:00.000Z',
  ...
  it('prints the payload checksum when the container has no agent state', async () => {
    ...
    expect(error.mock.calls.flat()).toContain(formatImportChecksum(checksum!));
  ...
  it('omits the checksum line when the container has no agent state and sha256 is empty', async () => {
    const { filePath } = await writeContainer('no-agent-empty-checksum.savestate', '   ');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
