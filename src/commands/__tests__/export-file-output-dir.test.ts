...
describe('savestate export file output directory', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-export-file-output-dir-${Date.now()}`);
  ...
  it('rejects a file output directory before writing a container', async () => {
    const parentFile = join(testDir, 'parent-file');
    await fs.writeFile(parentFile, 'not-a-directory');
    const filePath = join(parentFile, 'nested.savestate');
  ...
  it('still exports when the output directory is a directory', async () => {
