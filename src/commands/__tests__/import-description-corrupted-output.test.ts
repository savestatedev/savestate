...
describe('savestate import description corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-description-corrupted-${Date.now()}`);
  ...
  async function writeContainer(fileName: string, description?: unknown): Promise<string> {
    ...
    const manifest: Record<string, unknown> = {
      formatVersion: 1,
      created: '2026-08-24T16:00:00.000Z',
      agentId: 'fixture-agent',
      payloads: [
        {
          name: 'agent_state',
  ...
  it('prints the description when the file is corrupted', async () => {
    const filePath = await writeContainer('corrupted-description.savestate', 'Labeled import fixture');
  ...
  it('omits the description line when the file is corrupted and description is empty', async () => {
    const filePath = await writeContainer('corrupted-empty-description.savestate', '   ');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
