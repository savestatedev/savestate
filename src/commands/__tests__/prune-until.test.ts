...
import { parsePruneUntil, planPrune } from '../prune.js';
...
describe('savestate prune --until', () => {
  it('leaves the cutoff unset when omitted', () => {
    expect(parsePruneUntil(undefined)).toBeUndefined();
  });

  it('accepts ISO 8601 dates', () => {
    expect(parsePruneUntil('2026-06-01')).toBe(new Date('2026-06-01').getTime());
    expect(parsePruneUntil('2026-06-01T00:00:00Z')).toBe(
  ...
  it.each(['', ' ', 'nope', '2026-13-01', 'not-a-date'])(
    ...
    (value) => {
      expect(() => parsePruneUntil(value)).toThrow(
        `Invalid --until value "${value}". Expected an ISO 8601 date.`,
      );
    },
  );

  it('only drops snapshots taken before the cutoff', () => {
    const plan = planPrune(
      ...
      { keepLast: 1, untilMs: Date.parse('2026-06-01') },
    );
    expect(plan.drop.map((snapshot) => snapshot.id)).toEqual(['old']);
    expect(plan.keep.map((snapshot) => snapshot.id)).toEqual(['mid', 'new']);
  });
  ...
  it('ANDs --adapter with --until', () => {
    const plan = planPrune(
      ...
      { keepLast: 1, adapter: 'chatgpt', untilMs: Date.parse('2026-06-01') },
    );
    expect(plan.drop.map((snapshot) => snapshot.id)).toEqual(['old-gpt']);
    expect(plan.keep.map((snapshot) => snapshot.id)).toEqual(['mid-gpt', 'new-gpt', 'mid-claude']);
  });

  it('ANDs --until with --older-than', () => {
    const plan = planPrune(
      [
        entry({ id: 'old', adapter: 'chatgpt', timestamp: '2026-01-01T00:00:00Z' }),
        entry({ id: 'mid', adapter: 'chatgpt', timestamp: '2026-05-01T00:00:00Z' }),
      ...
      { olderThanMs: Date.parse('2026-04-01'), untilMs: Date.parse('2026-06-01') },
    );
    expect(plan.drop.map((snapshot) => snapshot.id)).toEqual(['old']);
    expect(plan.keep.map((snapshot) => snapshot.id)).toEqual(['mid', 'new']);
  });
