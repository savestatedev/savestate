...
describe('savestate import key derivation no-agent output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-kdf-no-agent-${Date.now()}`);
  ...
  async function writeContainer(fileName: string, encryption?: unknown): Promise<string> {
    ...
    const plaintext = Buffer.from(JSON.stringify({
      ...
      version: 1,
      exportedAt: '2026-08-25T13:00:00.000Z',
  ...
  it('prints the key derivation when the container has no agent state', async () => {
    ...
    expect(error.mock.calls.flat()).toContain(formatImportKeyDerivation('Argon2id'));
  ...
  it('omits the key derivation line when the container has no agent state and KDF is empty', async () => {
    const filePath = await writeContainer('no-agent-empty-kdf.savestate', {
      algorithm: 'AES-256-GCM',
      keyDerivation: '   ',
    });
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
