...
describe('savestate import agent corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-agent-corrupted-${Date.now()}`);
  ...
  async function writeContainer(fileName: string, agentId?: unknown): Promise<string> {
    ...
    const manifest: Record<string, unknown> = {
      formatVersion: 1,
      created: '2026-08-24T14:00:00.000Z',
      payloads: [
        {
          name: 'agent_state',
  ...
  it('prints the agent id when the file is corrupted', async () => {
    const filePath = await writeContainer('corrupted-agent.savestate', 'fixture-agent');
  ...
  it('omits the agent line when the file is corrupted and agent id is empty', async () => {
    const filePath = await writeContainer('corrupted-empty-agent.savestate', '   ');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
