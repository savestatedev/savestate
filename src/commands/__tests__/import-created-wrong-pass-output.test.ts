...
describe('savestate import created wrong-password output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-created-wrong-pass-${Date.now()}`);
  ...
  it('formats the created timestamp on its own line', () => {
    expect(formatImportCreated('2026-08-24T14:00:00.000Z')).toBe('  Created: 2026-08-24T14:00:00.000Z');
    expect(formatImportCreated('2026-08-24T14:00:00.000Z')).not.toContain('Agent:');
  });

  it('prints the created timestamp when the passphrase is wrong', async () => {
    const filePath = await writeContainer('wrong-pass-created.savestate', '2026-08-24T14:00:00.000Z');
    ...
    expect(error.mock.calls.flat()).toContain('  Created: 2026-08-24T14:00:00.000Z');
    expect(log.mock.calls.flat().some((line) => typeof line === 'string' && line.startsWith('  Created: '))).toBe(false);
    error.mockRestore();
    log.mockRestore();
  });

  it('omits the created line when the passphrase is wrong and created is empty', async () => {
    const filePath = await writeContainer('wrong-pass-empty-created.savestate', '   ');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
