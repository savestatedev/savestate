...
describe('savestate verify missing input path', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = await mkdtemp(join(tmpdir(), 'savestate-verify-missing-input-'));
  });
  ...
  it('rejects a missing input path before reading', async () => {
  ...
  it('still verifies an input path that exists', async () => {
