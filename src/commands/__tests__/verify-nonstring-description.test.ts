...
describe('savestate verify non-string description', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = await mkdtemp(join(tmpdir(), 'savestate-verify-nonstring-description-'));
  });
  ...
  async function writeContainer(
    ...
    const manifest: Record<string, unknown> = {
      formatVersion: 1,
      created: '2026-08-19T06:05:00.000Z',
      agentId: 'demo-agent',
      payloads: [
        {
          name: 'agent_state',
  ...
  it('rejects a container whose description is a number', async () => {
    const filePath = await writeContainer('description-number.savestate', 42);
  ...
  it('rejects a container whose description is a boolean', async () => {
    const filePath = await writeContainer('description-boolean.savestate', true);
  ...
  it('still verifies a valid container with a string description', async () => {
