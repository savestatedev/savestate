...
describe('savestate import description no-agent output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-description-no-agent-${Date.now()}`);
  ...
  async function writeContainer(fileName: string, description?: unknown): Promise<string> {
    ...
    const manifest: Record<string, unknown> = {
      formatVersion: 1,
      created: '2026-08-25T16:00:00.000Z',
      agentId: 'fixture-agent',
      payloads: [
        {
          name: 'other_payload',
  ...
  it('prints the description when the container has no agent state', async () => {
    const filePath = await writeContainer('no-agent-description.savestate', 'Labeled import fixture');
    ...
    expect(error.mock.calls.flat()).toContain(formatImportDescription('Labeled import fixture'));
  ...
  it('omits the description line when the container has no agent state and description is empty', async () => {
    const filePath = await writeContainer('no-agent-empty-description.savestate', '   ');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
