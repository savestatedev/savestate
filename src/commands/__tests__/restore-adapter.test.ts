...
import { parseRestoreAdapter, resolveRestoreSnapshot } from '../restore.js';

describe('savestate restore --adapter', () => {
  it('defaults to undefined', () => {
    expect(parseRestoreAdapter(undefined)).toBeUndefined();
  });

  it('accepts known adapters', () => {
    expect(parseRestoreAdapter('chatgpt')).toBe('chatgpt');
    expect(parseRestoreAdapter('claude-code')).toBe('claude-code');
    expect(parseRestoreAdapter('CLAUDE-WEB')).toBe('claude-web');
    expect(parseRestoreAdapter(' Windsurf ')).toBe('windsurf');
  });

  it.each(['', ' ', 'nope', 'claude', '1', 'chat gpt'])('rejects invalid value %s', (value) => {
    expect(() => parseRestoreAdapter(value)).toThrow(
