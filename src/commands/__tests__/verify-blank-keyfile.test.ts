...
describe('savestate verify blank keyfile', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = await mkdtemp(join(tmpdir(), 'savestate-verify-blank-keyfile-'));
  });
  ...
  it('rejects a 0-byte keyfile before decrypting', async () => {
    const filePath = await writeContainer('empty-body-keyfile.savestate', {
      passphrase: 'synthetic-verify-pass',
    });
    const keyfilePath = join(testDir, 'empty.key');
    ...
    expect(result.message).toMatch(/contents/i);
    expect(result.checksum).toBeUndefined();
    expect(output).toMatch(/keyfile/i);
    expect(output).toMatch(/contents/i);
  ...
  it('rejects a whitespace-only keyfile before decrypting', async () => {
    const filePath = await writeContainer('whitespace-body-keyfile.savestate', {
      passphrase: 'synthetic-verify-pass',
    });
    const keyfilePath = join(testDir, 'whitespace.key');
    await writeFile(keyfilePath, '   \n');
  ...
  it('still verifies a non-empty keyfile', async () => {
