...
describe('savestate import size wrong-password output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-size-wrong-pass-${Date.now()}`);
  ...
  it('prints the payload size when the passphrase is wrong', async () => {
    const { filePath, payloadBytes } = await writeContainer('wrong-pass-size.savestate');
    ...
    expect(error.mock.calls.flat()).toContain(`  Size: ${payloadBytes} bytes`);
    expect(log.mock.calls.flat().some((line) => typeof line === 'string' && line.startsWith('  Size:'))).toBe(false);
    error.mockRestore();
    log.mockRestore();
  });

  it('omits the size line when the passphrase is wrong and byteLength is missing', async () => {
    const { filePath } = await writeContainer('wrong-pass-missing-size.savestate', null);
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
