...
describe('savestate import content type no-agent output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-content-type-no-agent-${Date.now()}`);
  ...
  it('prints the content type when the container has no agent state', async () => {
    const filePath = await writeContainer('no-agent-content-type.savestate', 'application/json');
    ...
    expect(error.mock.calls.flat()).toContain(formatImportContentType('application/json'));
  ...
  it('omits the content type line when the container has no agent state and content type is empty', async () => {
    const filePath = await writeContainer('no-agent-empty-content-type.savestate', '   ');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
