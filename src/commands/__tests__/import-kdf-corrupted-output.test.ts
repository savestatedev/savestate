...
describe('savestate import key derivation corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-kdf-corrupted-${Date.now()}`);
  ...
  it('prints the key derivation when the file is corrupted', async () => {
    const filePath = await writeContainer('corrupted-kdf.savestate', {
  ...
  it('omits the key derivation line when the file is corrupted and KDF is empty', async () => {
    const filePath = await writeContainer('corrupted-empty-kdf.savestate', {
      algorithm: 'AES-256-GCM',
      keyDerivation: '   ',
    });
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
