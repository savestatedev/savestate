...
describe('savestate import target corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-target-corrupted-${Date.now()}`);
  ...
  it('prints the target path when the file is corrupted', async () => {
    const filePath = await writeContainer('corrupted-target.savestate');
  ...
  it('omits a target line when the file is corrupted and --target is not set', async () => {
    const filePath = await writeContainer('corrupted-no-target.savestate');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
