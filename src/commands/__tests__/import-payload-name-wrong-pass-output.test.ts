...
describe('savestate import payload name wrong-password output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-payload-name-wrong-pass-${Date.now()}`);
  ...
  it('prints the payload name when the passphrase is wrong', async () => {
    const filePath = await writeContainer('wrong-pass-payload-name.savestate', 'agent_state');
  ...
  it('omits the payload name line when the passphrase is wrong and name is empty', async () => {
    const filePath = await writeContainer('wrong-pass-empty-payload-name.savestate', '   ');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
