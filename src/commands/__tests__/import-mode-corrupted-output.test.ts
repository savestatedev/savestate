...
describe('savestate import mode corrupted output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-mode-corrupted-${Date.now()}`);
  ...
  it('prints replace mode when the file is corrupted', async () => {
    const filePath = await writeContainer('corrupted-mode.savestate');
  ...
  it('prints merge mode when the file is corrupted and merge is requested', async () => {
    const filePath = await writeContainer('corrupted-merge-mode.savestate');
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const log = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const exit = vi.spyOn(process, 'exit').mockImplementation(((code?: number) => {
      throw new Error(`process.exit:${code ?? ''}`);
