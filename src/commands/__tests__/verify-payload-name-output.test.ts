...
describe('savestate verify payload name output', () => {
  let testDir: string;
  ...
  it('returns the payload name after a valid checksum comparison', async () => {
    const passphrase = 'synthetic-verify-pass';
    const plaintext = Buffer.from(JSON.stringify({ memory: ['demo'], tools: [] }));
    const encryptedState = await encrypt(plaintext, { passphrase });
    const manifest = {
      formatVersion: 1,
      created: '2026-08-22T19:00:00.000Z',
      agentId: 'demo-agent',
      payloads: [
        {
          name: 'agent_state',
    ...
    expect(formatVerifyPayloadName(result.payloadName!)).toBe('   Payload: agent_state');
  ...
  it('prints a Payload line for a known name', () => {
    expect(formatVerifyPayloadName('agent_state')).toBe('   Payload: agent_state');
    expect(formatVerifyPayloadName('agent_state')).not.toContain('Agent:');
  });
});
