...
describe('savestate import encryption no-agent output', () => {
  let testDir: string;

  beforeAll(async () => {
    testDir = join(tmpdir(), `savestate-import-encryption-no-agent-${Date.now()}`);
  ...
  it('prints the encryption algorithm when the container has no agent state', async () => {
    ...
    expect(error.mock.calls.flat()).toContain(formatImportEncryption('AES-256-GCM'));
  ...
  it('omits the encryption line when the container has no agent state and algorithm is empty', async () => {
    const filePath = await writeContainer('no-agent-empty-encryption.savestate', {
      algorithm: '   ',
      keyDerivation: 'Argon2id',
    });
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined);
